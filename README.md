...
The `[1m]` arrives from session configuration, so no frontmatter suffix is
needed. One caveat on that evidence: a model reads its own ID from its system
prompt rather than from the routing layer.
